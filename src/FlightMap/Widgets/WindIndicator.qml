...
Item {
    ...
    property bool _hasVariance:   _hasWind && !isNaN(_windVariance) && _windVariance > 0 && _windSpeed >= 0.1
    /// Angular uncertainty (degrees): atan(2σ / speed), clamped to [5, 180]
    // _windVariance is variance (m²/s²), so take sqrt to get σ (m/s)
    property real _angleUncertainty: {
        if (!_hasVariance) return 0
        var sigma = Math.sqrt(_windVariance)
        var angleDeg = Math.atan2(2 * sigma, _windSpeed) * 180 / Math.PI
        return Math.max(5, Math.min(angleDeg, 180))
    }
    ...
    Canvas {
        ...
        onPaint: {
            ...
            // Draw uncertainty sector only when variance data is available
            if (_hasVariance) {
                var halfSpread = _angleUncertainty * Math.PI / 360
                ctx.beginPath()
                ctx.moveTo(cx, cy)
    ...
    Connections {
        ...
        function on_angleUncertaintyChanged() { windCanvas.requestPaint() }
        function on_hasWindChanged()          { windCanvas.requestPaint() }
        function on_hasVarianceChanged()      { windCanvas.requestPaint() }
    }
    ...
    Rectangle {
        ...
        QGCLabel {
            ...
            text: {
                if (!_hasWind) return "---"
                if (!_hasVariance) return _windSpeed.toFixed(1)
                var uncertainty = Math.sqrt(_windVariance) * 2
                var lo = Math.max(0, _windSpeed - uncertainty).toFixed(1)
                var hi = (_windSpeed + uncertainty).toFixed(1)
                return lo + " - " + hi
            }
        }
    }
