...
void VehicleBatteryFactGroup::updateTimeUntilNextThreshold()
{
    ...
    double timeRemaining = _timeRemainingFact.rawValue().toDouble();

    if (qIsNaN(currentMahConsumed) || qIsNaN(maxCapacity) || qIsNaN(timeRemaining) || timeRemaining <= 0) {
        _timeUntilNextThresholdFact.setRawValue(qQNaN());
    ...
    for (const auto& threshold : _thresholds) {
        if (currentMahConsumed < threshold.mahThreshold) {
            double estimatedAverageCurrent = timeRemaining / mahRemaining;
            double timeUntilThreshold = (threshold.mahThreshold - currentMahConsumed) / estimatedAverageCurrent;
            _timeUntilNextThresholdFact.setRawValue(timeUntilThreshold);
